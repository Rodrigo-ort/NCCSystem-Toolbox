function handles = barweb(barvalues, errors, width, groupnames, bw_title, ...
    bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type, ...
    fontsize_plus)
%
...
%
% barweb calls the MATLAB bar function and plots m groups of n bars using
% the width and bw_colormap parameters.
%
% If you want all the bars to be the same color, then set bw_colormap equal
% to the RBG matrix value ie. (bw_colormap = [1 0 0] for all red bars)
%
% barweb then calls the MATLAB errorbar function to draw barvalues with error bars of length error.
%
% Groupnames is an m-length cellstr vector of groupnames
% (i.e. groupnames = {'group 1'; 'group 2'}).  For no groupnames, enter [] or {}
%
...
% legend_type = 'axis' produces the legend along the x-axis while
% legend_type = 'plot' produces the standard legend.  See figure for more details
%
...
% handles.errors: a vector of handles to the error plots, with each handle
% corresponding to a column in the error matrix
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    if ~isempty(bw_colormap)
        ...
        %colormap(bw_colormap(3));
    else
        colormap(jet);
    end
    if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
        handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',12+fontsize_plus);
        legend boxoff;
    else
        handles.legend = [];
    end
    ...
    if strcmp(legend_type, 'axis')
        for i = 1:numbars
            ...
            for j = 1:length(xdata)
                text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60,...
                    'fontsize', 12+fontsize_plus, 'HorizontalAlignment', 'right');
            end
        end
    ...
    if ~isempty(bw_title)
        title(bw_title, 'fontsize',14+fontsize_plus);
    end
    if ~isempty(bw_xlabel)
        xlabel(bw_xlabel, 'fontsize',14+fontsize_plus);
    end
    if ~isempty(bw_ylabel)
        ylabel(bw_ylabel, 'fontsize',14+fontsize_plus);
    end
    
    set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], ...
        'fontsize', 12+fontsize_plus, 'xtick',1:numgroups, 'linewidth', 2,'xgrid','off','ygrid','off');
