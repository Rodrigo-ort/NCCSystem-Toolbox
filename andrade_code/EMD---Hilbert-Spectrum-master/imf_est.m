...
function [i_m_f,is_imf]= imf_est(x,IsIMF,interpMethod,IMF_ID)
    ...
    while (stop_cond1 >= 2),
        ...
        % oliveirafhm modification to avoid infinity loop when average env
        % is zero and zero-crossing - extreme is two
        if stop_cond2 < 0.0000001
            fprintf('Break...oliveirafhm mod...\n'); % Test
            break;
            %             fprintf('Return...oliveirafhm mod...\n'); % Test
            %             return;
        end
    end;%while
