...
function [] = EstimateTFplots(EMG,fs,NLevels)
...
CH = contour(time,f,m_a_p,NLevels); %Estimating contour plots (9 levels are adopted as default)
...
subplot(5,1,3); 
CF = contour(t_F,f_F,B_F,NLevels);colormap hot;set(gca,'Color','k') ;
ylabel('Hz'); xlabel('time (s)');
XtickLabels = str2num(get(gca,'XtickLabel')); XtickLabels(end+1) = max(t_F);
set(gca,'Xtick',XtickLabels);
XtickLabels(end+1) = t(end);
set(gca,'XtickLabel',num2str(XtickLabels,'%0.2f'));
set(gca,'Ylim',[min(get(gca,'YLim')) 1000]);
...
%Plotting
subplot(5,1,4); 
[Cwav] = contour((1:length(EMG))/fs,Fwav,Scal,NLevels);
...
%Displaying results
subplot(5,1,5);
contour((1:length(EMG))/fs,Freq,TFR,NLevels);
