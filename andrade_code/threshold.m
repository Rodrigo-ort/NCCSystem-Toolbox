...
% Remarks: this threshold is valid for the complex envolope of the signal but not for the
%          raw EMG signal

function [th,yenv]= threshold(y,Nbins)

%Estimating the complex envelope of the time series based on the hilbert transform
yenv = abs(hilbert(y));

%Estimating the histogram
[n,xout] = hist(abs(y),Nbins); %number of bins = 2^9
...
%Estimating the (position of the) standard deviation
[index] = find(n(Index_m:end)<=0.61*m);
standardDev = xout(index(1));
%Estimating threshold
th = 5*standardDev; %based on the Chebychev's theorem