...
% Filter by R value to exclude outliers (test...)
xlsData = xlsData(abs(xlsData(:,7)) > 0.35,:);
% Obs.: 122 valid samples
...
nTasks = 8;
nAxis = 2;
corrSamplesN = ceil(max([length(xIdx) length(yIdx)]) / nTasks);
...
%     nSamplesTask = length(axisIdx{ax}) / nTasks;
    for t = 1:nTasks
        ...
        corrMatrix(1:length(tIdx), ax, t) = abs(axisData{ax}(tIdx, 7));
    end
end
